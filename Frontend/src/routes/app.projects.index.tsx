...
function Projects() {
  ...
  return (
    <div>
      <PageHeader title="Projects" subtitle="Coordinate work across teams with AI-powered insights." actions={<Button className="gradient-primary text-primary-foreground border-0" onClick={() => setOpen(true)}><Plus className="h-4 w-4 mr-2" />New Project</Button>} />
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        {projects.map((p) => {
          const openCount = tasks.filter(t => t.project === p.id && t.status !== "done").length;
          return (
            <Card key={p.id} className="p-5 bg-card/60 border-border/60 hover:border-primary/40 transition cursor-pointer" onClick={() => navigate({ to: "/app/projects/$id", params: { id: p.id } })}>
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                <div className="text-xs text-muted-foreground">{openCount} open • due {format(new Date(p.due),"MMM d")}</div>
              </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <form onSubmit={async (e) => {
            ...
            <div className="space-y-1.5"><Label>Name</Label><Input name="name" required defaultValue="Enterprise Launch" /></div>
            <div className="space-y-1.5"><Label>Description</Label><Textarea name="description" required defaultValue="Coordinate launch readiness across product, engineering, sales, and success." /></div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5"><Label>Progress</Label><Input name="progress" type="number" min="0" max="100" defaultValue={5} /></div>
              <div className="space-y-1.5"><Label>Due date</Label><Input name="due" type="date" defaultValue={new Date(Date.now()+86400000*30).toISOString().slice(0,10)} /></div>
            </div>
            <DialogFooter>
              <Button type="button" variant="ghost" onClick={() => setOpen(false)}>Cancel</Button>
              <Button type="submit" className="gradient-primary text-primary-foreground border-0">Create project</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
