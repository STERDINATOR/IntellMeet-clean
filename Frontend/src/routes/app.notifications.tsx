        ...
        {["all","meeting","task","mention","ai","workspace"].map(t=>(
          <TabsContent key={t} value={t} className="mt-4 space-y-2">
            {filter(t).map(n => { const Icon = icons[n.type] ?? Bell; return (
              <Card key={n.id} className={`p-4 bg-card/60 border-border/60 flex items-start gap-3 cursor-pointer ${!n.read?"border-l-4 border-l-primary":""}`} onClick={()=>markRead(n.id)}>
