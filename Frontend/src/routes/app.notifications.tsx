...
function Notifications() {
  ...
  const filter = (type: string) =>
    type === "all" ? items : items.filter((n) => n.type === type);

  useEffect(() => {
    notificationService
      .list()
      .catch(() => toast.error("Unable to fetch notifications."));
  }, []);
          ...
          <TabsContent key={t} value={t} className="mt-4 space-y-2">
            {filter(t).map((n) => {
              ...
              return (
                <Card
                  key={n.id}
                  className={`p-4 bg-card/60 border-border/60 flex items-start gap-3 cursor-pointer ${!n.read ? "border-l-4 border-l-primary" : ""}`}
                  onClick={async () => {
                    await notificationService
                      .markRead(n.id)
                      .catch(() =>
