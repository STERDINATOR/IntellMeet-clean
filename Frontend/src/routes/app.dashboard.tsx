...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    analyticsService
      ...
      .then((d) => {
        setLiveAnalytics(d);
        setLiveInsights(d.aiRecommendations ?? []);
      })
      .catch(() => undefined);
    analyticsService
      .recentActivity()
      .then(setLiveActivity)
      .catch(() => undefined);
  }, []);

  // Refresh analytics when tasks/meetings update via realtime
  useEffect(() => {
    const unsub = realtimeBus.on("task:updated", () => {
      analyticsService
        .dashboard()
        .then((d) => {
          setLiveAnalytics(d);
        ...
        <Card className="lg:col-span-2 p-6 bg-card/60 border-border/60">
          ...
          <div className="space-y-2">
            {upcoming.map((m) => (
              <div
                key={m.id}
                className="flex items-center gap-3 rounded-xl p-3 hover:bg-secondary/60 transition cursor-pointer"
                onClick={() =>
                  navigate({ to: "/app/room/$id", params: { id: m.id } })
                }
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {m.duration}min • {m.type} • {m.participants.length}{" "}
                    participants
                  </div>
                </div>
        ...
        <Card className="p-6 bg-card/60 border-border/60 relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <div className="text-xs text-muted-foreground mb-3">
              Design Critique • 1h ago
            </div>
            <p className="text-sm leading-relaxed">
              Team aligned on Aurora v2 tokens. Maya to ship the audit by
              Wednesday; Noah to wire the new theme into the room. 3 decisions,
              4 action items, 1 risk.
            </p>
            ...
            <Button
              ...
              className="px-0 mt-2"
              onClick={() =>
                navigate({ to: "/app/meetings/$id", params: { id: "m4" } })
              }
            >
              Open summary <ArrowRight className="h-3 w-3 ml-1" />
            </Button>
          </div>
        </Card>
      </div>
        ...
        <Card className="p-6 bg-card/60 border-border/60">
          ...
          <div className="space-y-2">
            {recent.map((m) => (
              <div
                key={m.id}
                className="flex items-center gap-3 hover:bg-secondary/60 p-2 rounded-lg cursor-pointer"
                onClick={() =>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {format(new Date(m.start), "MMM d, HH:mm")}
                  </div>
                </div>
              ...
              <div key={a.id} className="flex gap-3 text-sm">
                <Avatar className="h-7 w-7">
                  <AvatarImage
                    src={
                      a.actor.avatar ??
                      `https://api.dicebear.com/9.x/glass/svg?seed=${a.actor.name}`
                    }
                  />
        ...
        <DialogContent>
          ...
          <form
            onSubmit={(e) => {
              ...
              startInstant(String(f.get("title") || ""));
            }}
            className="space-y-4"
          >
            <div className="space-y-1.5">
              <Label>Meeting title</Label>
              <Input name="title" placeholder="Quick sync" autoFocus />
            </div>
            <DialogFooter>
              <Button
                type="button"
