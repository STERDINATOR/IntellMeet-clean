...
function Analytics() {
  type AnalyticsDashboard = Awaited<
    ReturnType<typeof analyticsService.dashboard>
  >;
  const [liveData, setLiveData] = useState<AnalyticsDashboard | null>(null);
  ...
  return (
    <div>
      <PageHeader
        title="Analytics"
        subtitle="Meeting trends, productivity, and effectiveness."
        actions={
          <Button
            variant="outline"
            onClick={async () => {
        ...
        <Card className="lg:col-span-3 p-6 bg-card/60 border-border/60 relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <div className="grid md:grid-cols-3 gap-3">
              {aiRecs.filter(Boolean).map((s: string) => (
                <div key={s} className="rounded-xl bg-secondary/40 p-4 text-sm">
                  {s}
                </div>
              ))}
            </div>
