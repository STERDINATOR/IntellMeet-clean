        ...
        {reports.map(r=>(
          <Card key={r.name} className="p-5 bg-card/60 border-border/60">
            ...
            <div className="mt-4 flex gap-2">
              <Button variant="outline" size="sm" className="flex-1" onClick={()=>toast.success(`${r.name} PDF exported`)}><Download className="h-3 w-3 mr-1" />PDF</Button>
              <Button variant="outline" size="sm" className="flex-1" onClick={()=>toast.success(`${r.name} Excel exported`)}><Download className="h-3 w-3 mr-1" />Excel</Button>
            </div>
          </Card>
        ))}
      </div>
