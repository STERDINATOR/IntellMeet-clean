  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <>
            <Button variant="outline" onClick={instant}>
        ...
        <TabsContent value={filter}>
          <div className="grid lg:grid-cols-2 gap-4">
            {list.map((m) => (
              <Card
                key={m.id}
                className="p-5 bg-card/60 border-border/60 hover:border-primary/40 transition group"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
                      ...
                      {format(new Date(m.start), "EEE, MMM d • HH:mm")} •{" "}
                      {m.duration}min
                    </div>
                    {m.agenda && (
                  ...
                  <div className="text-2xl font-bold text-primary opacity-60 group-hover:opacity-100">
                    {m.score || "—"}
                  </div>
                </div>
                <div className="flex items-center justify-between mt-4">
                  <div className="flex -space-x-2">
                    ...
                    {m.participants.length > 4 && (
                      <div className="h-7 w-7 rounded-full bg-secondary ring-2 ring-background flex items-center justify-center text-xs">
                        +{m.participants.length - 4}
                      </div>
                    )}
                  </div>
