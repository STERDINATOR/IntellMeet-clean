...
import { findUser, users as mockUsers } from "@/lib/mock";
...
function Tasks() {
  ...
  const [teamUsers, setTeamUsers] = useState(mockUsers);
    ...
    userService
      ...
      .then((data) => {
        if ((data as any[]).length)
          setTeamUsers(
            (data as any[]).map((u) => ({ ...u, id: u.id ?? u._id })),
          );
      })
      .catch(() => undefined);
  }, []);
    ...
    <div>
      ...
      <Sheet open={!!open} onOpenChange={(o) => !o && setOpen(null)}>
        <SheetContent className="w-[480px] sm:max-w-[480px]">
          {open &&
            (() => {
              const u = findUser(open.assignee);
              return (
                <>
                  <SheetHeader>
