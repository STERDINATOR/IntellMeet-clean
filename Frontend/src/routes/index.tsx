...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      {
        property: "og:title",
        content: "IntellMeet — AI-Powered Collaboration",
      },
      {
        property: "og:description",
        content: "Meet smarter. Collaborate better. Powered by AI.",
      },
    ],
  }),
  component: Landing,
    ...
    <div className="min-h-screen bg-background text-foreground relative overflow-hidden">
        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link to="/login">
              <Button
                variant="outline"
                className="rounded-full h-10 px-5 border-border/80"
              >
                Log in
              </Button>
            </Link>
            <Link to="/signup">
              <Button className="rounded-full h-10 px-5 gradient-primary text-primary-foreground border-0 glow">
                Get Started
              </Button>
            </Link>
          </div>
      ...
      <section className="relative max-w-[1400px] mx-auto px-6 lg:px-10 pt-10 pb-20">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <h1 className="mt-8 text-5xl md:text-6xl lg:text-7xl font-bold leading-[1.05] tracking-tight">
              Meet Smarter.
              <br />
              Collaborate <span className="gradient-text">Better.</span>
              <br />
              Powered by <span className="gradient-text">AI.</span>
            </h1>
            <p className="mt-7 text-lg text-muted-foreground max-w-xl leading-relaxed">
              Real-time meetings, AI summaries, smart action items, and team
              collaboration — all in one intelligent workspace.
            </p>
            <div className="mt-9 flex flex-wrap gap-3">
              <Link to="/signup">
                <Button
                  size="lg"
                  className="h-12 px-7 rounded-xl gradient-primary text-primary-foreground border-0 glow text-base"
                >
              ...
              <Button
                ...
                variant="outline"
                onClick={() => setDemoOpen(true)}
                className="h-12 px-7 rounded-xl border-border/80 text-base"
              >
        ...
        <div className="mt-24 text-center">
          ...
          <div className="mt-8 flex flex-wrap justify-center items-center gap-x-14 gap-y-5 opacity-80">
            {brands.map((b) => (
              <div key={b} className="text-2xl font-semibold tracking-tight">
                {b}
              </div>
            ))}
          </div>
      ...
      <footer className="border-t border-border/40 py-8 mt-10 relative">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10 flex flex-wrap items-center justify-between gap-3 text-sm text-muted-foreground">
          <div className="flex items-center gap-2">
            <div className="h-5 w-5 rounded gradient-primary" /> © 2026
            IntellMeet, Inc.
          </div>
      ...
      <Dialog open={demoOpen} onOpenChange={setDemoOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>IntellMeet demo</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Preview the end-to-end flow: schedule a meeting, join the live
              room, generate AI action items, and export reports.
            </p>
            <Link to="/signup">
