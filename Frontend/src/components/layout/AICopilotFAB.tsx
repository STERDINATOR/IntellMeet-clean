...
export function AICopilotFAB() {
  ...
  >([
    {
      role: "ai",
      text: "Hi Alex, I'm your IntellMeet Copilot. Ask me anything about your workspace.",
    },
  ]);
    ...
    try {
      await streamAI({
        ...
        onDelta: (delta) =>
          setMessages((m) =>
            m.map((item, index) =>
              index === aiIndex ? { ...item, text: item.text + delta } : item,
            ),
          ),
      });
    } catch {
      setMessages((m) =>
        m.map((item, index) =>
          index === aiIndex ? { ...item, text: fallbackReply(text) } : item,
        ),
      );
    } finally {
      setTyping(false);
          ...
          >
            ...
            >
              {messages.map((m, i) => (
                ...
                >
                  <div
                    className={`max-w-[85%] rounded-2xl px-3 py-2 text-sm whitespace-pre-wrap ${m.role === "user" ? "bg-primary text-primary-foreground" : "bg-secondary"}`}
                  >
                    {m.text ||
              ...
              {typing && (
                <div className="flex">
                  <div className="bg-secondary rounded-2xl px-3 py-2 text-sm flex gap-1">
            ...
            <div className="px-4 pb-2 flex flex-wrap gap-1.5">
              {quick.map((q) => (
                <button
                  key={q}
                  onClick={() => send(q)}
                  className="text-[11px] px-2 py-1 rounded-full bg-secondary hover:bg-secondary/70 text-muted-foreground hover:text-foreground transition flex items-center gap-1"
                >
