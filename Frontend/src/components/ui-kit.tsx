  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
      <Card className="relative overflow-hidden p-5 border-border/60 bg-card/60 backdrop-blur">
        <div
          className={cn(
            "absolute -top-10 -right-10 h-32 w-32 rounded-full blur-3xl bg-gradient-to-br",
            accentMap[accent],
          )}
        />
        <div className="relative flex items-start justify-between">
          <div>
            ...
            {change && (
              <div className="text-xs text-success mt-1">{change}</div>
            )}
          </div>
          <div
            className={cn(
              "h-10 w-10 rounded-xl bg-secondary/80 flex items-center justify-center",
