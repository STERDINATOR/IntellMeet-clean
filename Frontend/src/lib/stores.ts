...
import { currentUser, tasks as seedTasks, meetings as seedMeetings, notifications as seedNotifications, projects as seedProjects, type Task, type Meeting, type Notification, type Project, type User } from "./mock";
...
export const useAuthStore = create<{
  ...
  login: () => void;
  setSession: (session: { user?: Partial<User> & { _id?: string }; accessToken: string; refreshToken: string }) => void;
  logout: () => void;
}>()(persist((set) => ({
  isAuthed: false,
  user: currentUser,
  ...
  setSession: ({ user, accessToken, refreshToken }) => set({
    isAuthed: true,
    user: { ...currentUser, ...user, id: user?._id ?? user?.id ?? currentUser.id } as User,
    accessToken,
    refreshToken,
  }),
  logout: () => set({ isAuthed: false, user: currentUser, accessToken: null, refreshToken: null }),
}), { name: "im-auth" }));
...
export const useMeetingsStore = create<{
  ...
  update: (id, patch) => set((s) => ({ meetings: s.meetings.map(x => x.id === id ? { ...x, ...patch } : x) })),
...
export const useNotificationsStore = create<{
  ...
  add: (n) => set((s) => ({ items: [{ ...n, id: `n${Date.now()}`, time: new Date().toISOString(), read: false }, ...s.items] })),
